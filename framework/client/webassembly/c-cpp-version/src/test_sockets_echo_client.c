...
void main_loop() {
  ...
  if (server.state == MSG_READ) {
	...
	printf("do_msg_read echo_read = %d, server.msg.length = %d\n", echo_read, server.msg.length);
  ...
  if (server.state == MSG_WRITE) {
	...
	printf("do_msg_write echo_wrote = %d, echo_msg.length = %d\n", echo_wrote, echo_msg.length);
...
int main() {
...

  {
    ...
    sprintf(buffer, "%s:%u", inet_ntoa(adr_inet.sin_addr), (unsigned)ntohs(adr_inet.sin_port));
    char correct[1000];
    sprintf(correct, "127.0.0.1:%u", 7777);
    printf("got (expected) socket: %s (%s), size %lu (%lu)\n", buffer, correct, strlen(buffer), strlen(correct));
    assert(strlen(buffer) == strlen(correct));
    assert(strcmp(buffer, correct) == 0);
  }
...
#else
	printf("emscripten_set_main_loop before\n");
